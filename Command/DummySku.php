...
class DummySku extends Command
{
            ...
            if (is_numeric($iterations) && $iterations > 0) {
                for ($i = 0; $i < $iterations; $i++) {
                    ...
                    $product
                        ...
                        ->setTaxClassId($this->getRandomTaxClassId())
     ...
     */
    protected $cloneProduct;

    protected function getProductRandom($cloneProductId)
    {
        if (is_null($this->cloneProduct)) {
            $this->cloneProduct = $this->getProductModel()->load($cloneProductId);

            if (!$this->cloneProduct->getId()) {
                throw new Exception("check clone product id");
            }
        }

        return $this->cloneProduct->getData();
    }
